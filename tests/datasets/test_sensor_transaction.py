...
from lerobot.datasets import SensorTransaction as ExportedSensorTransaction
...
def test_formal_transaction_class_and_journal_version(tmp_path):
    recorder, transaction, uid = prepared_recorder(tmp_path)
    loaded = SensorTransaction.load(tmp_path / "meta/sensor_transactions" / f"{uid}.json")
    assert ExportedSensorTransaction is SensorTransaction
    assert type(transaction) is SensorTransaction
    assert type(loaded) is SensorTransaction
    assert loaded.journal["journal_version"] == JOURNAL_VERSION == 1
...
@pytest.mark.parametrize(
    "version",
    [None, 2, True, 1.0, "1"],
    ids=["missing", "prototype-v2", "boolean", "float", "string"],
)
def test_load_rejects_unsupported_journal_versions(tmp_path, version):
    recorder, transaction, uid = prepared_recorder(tmp_path)
    path = tmp_path / "meta/sensor_transactions" / f"{uid}.json"
    journal = json.loads(path.read_text())
    if version is None:
        del journal["journal_version"]
    else:
        journal["journal_version"] = version
    path.write_text(json.dumps(journal))
    with pytest.raises(SensorTransactionError, match="Unsupported sensor journal version"):
        SensorTransaction.load(path)
    recorder.abort_prepared("test finished")
    recorder.close()
...
def test_recovery_requires_writer_lock(tmp_path):
    ...
    with pytest.raises(SensorTransactionError, match="writer lock"):
        SensorTransaction.begin(tmp_path, str(uuid.uuid4()), 0)
...
def test_unidentified_nonempty_staging_is_refused_without_mutation(tmp_path):
    initialize_main(tmp_path)
    staging = tmp_path / ".sensor-staging" / str(uuid.uuid4())
    staging.mkdir(parents=True)
    (staging / "unknown.bin").write_bytes(b"unidentified")
    before = snapshot(tmp_path)
    with (
        SensorDatasetWriterLock(tmp_path) as lock,
        pytest.raises(SensorTransactionError, match="Unidentified staging"),
    ):
        TransactionRecoveryManager(tmp_path).recover(writer_lock=lock)
    assert snapshot(tmp_path) == before
...
def test_main_saved_staging_digest_corruption_is_quarantined(tmp_path):
    recorder, transaction, uid = prepared_recorder(tmp_path)
    dataset = FakeDataset(tmp_path)
    dataset._sensor_transaction = transaction
    dataset.save_episode()
    transaction.mark_main_saved()
    staging = tmp_path / transaction.journal["files"][0]["staging_path"]
    staging.write_bytes(b"corrupt")
    assert transaction.replay() == TransactionState.QUARANTINED
    assert (tmp_path / ".sensor-quarantine" / uid).exists()
    recorder._reset_episode()
    recorder.close()
...
def test_append_shared_main_artifacts_preserves_prior_logical_evidence(tmp_path):
    recorder, transaction, _uid = prepared_recorder(tmp_path)
...
def test_recovery_finishes_partial_committed_staging_cleanup(tmp_path):
    recorder, transaction, uid = prepared_recorder(tmp_path)
    recorder.commit_prepared(FakeDataset(tmp_path))
    staging = tmp_path / ".sensor-staging" / uid
    staging.mkdir(parents=True)
    (staging / "orphan.tmp").write_text("left after interrupted cleanup")
    pointer = tmp_path / ".sensor-staging/active_transaction.json"
    pointer.write_text(json.dumps({"episode_uid": uid, "intent_id": transaction.journal["intent_id"]}))

    recovered = TransactionRecoveryManager(tmp_path).recover(writer_lock=recorder._writer_lock)

    assert recovered.state == TransactionState.COMMITTED
...
def test_open_read_and_hash_work_is_independent_of_unrelated_history(tmp_path, monkeypatch):
    import lerobot.datasets.sensor_stream as stream

    original_open, original_hash = Path.open, transaction_module.sha256_file
    counts = {}
    ...
    for history in (0, 100):
        ...
        with monkeypatch.context() as patch:
            ...
            patch.setattr(transaction_module, "sha256_file", metered_hash)
