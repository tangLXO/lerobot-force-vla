...
def state_and_action_features():
    return (
        {OBS_STATE: PolicyFeature(type=FeatureType.STATE, shape=(SENSOR_STATE_DIM,))},
        {ACTION: PolicyFeature(type=FeatureType.ACTION, shape=(ACTION_DIM,))},
    )


def test_act_training_batch_accepts_sensor_extended_observation_state() -> None:
    ...
    batch = {
        OBS_STATE: torch.randn(2, SENSOR_STATE_DIM),
...
def test_pi0_and_smolvla_keep_standard_state_schema_shape() -> None:
    for config_cls in (PI0Config, SmolVLAConfig):
        input_features, output_features = state_and_action_features()
        config = config_cls(input_features=input_features, output_features=output_features)
        assert config.robot_state_feature == PolicyFeature(type=FeatureType.STATE, shape=(SENSOR_STATE_DIM,))
